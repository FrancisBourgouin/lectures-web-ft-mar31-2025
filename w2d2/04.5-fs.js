...
fs.readFile("./randomText.txt", { encoding: "utf8" }, (err, data) => {
  ...
  // Will either be step 5
  console.log(err, data);
  currentContent = data;

  console.log("About to write to a file"); // 6 - Log the message to the console

  // 7 - Call writeFile to write the content data + someCoolText
  fs.writeFile("./randomText.txt", currentContent + someCoolText, () => {
    // Will either be step 8
    console.log("Done writing to a file");
  });
});

// 4 - Every synchronous functions have been called
